...
// Used AI to help with the schema
...
app.post("/api/login", async (req, res, next) => {
  try {
    ...
    const receivedKeys = Object.keys(reqUser);
    ...
    // I had to use Copilot and the textbook examples to figure this out after many attempts.
    let user;
    try {
      user = await users.findOne({ email: reqUser.email });
    } catch (err) {
      console.error("User not found");
      return next(createError(401, "Unauthorized"));
    }

    const validPass = bcrypt.compareSync(reqUser.password, user.password);
